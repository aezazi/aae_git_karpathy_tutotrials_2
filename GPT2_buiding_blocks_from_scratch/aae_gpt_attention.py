...
batch_size = 16
block_size = 32 # sequence length or number of tokens
max_iters = 5000
eval_interval = 100 #number of iterations during training at which we evaluate how model is doing
learning_rate = 1e-3
eval_iters = 200
n_embd = 64 # dimension of the input embedding. Karpathy uses C for channels elsewhere in the code
n_head = 4
n_layer = 4
dropout = 0.0
...
# %%
"""
this is code by karpathy that tracks the loss as training proceeds. Don't fully understand how
it works, but what he is doing is that every so often he computes the training loss AND THE VALIDATION
LOSS. I hadn't seen this before. Usually, I've seen reporting just the training loss. 
"""
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, x):
        """
        MultiHeadAttention is simply running input x through the desired number of heads and
        concatenating the output. concatenation is across C (the dimensions of the embedding). 
        After concatention, the sum along the embedding dimension must equal n_embd for 
        further processing. So the head_size * num_heads must equal n_embd.
        """
...
#%%
"""
This are simple feed forward linear and sctivations layers. They are applied after MultiHeadAttention
As karpathy describes it, after multiattention, the tokens have gathered information from other tokens,
but they "haven't thought about what they found from the other tokens"
"""
...
class BigramLanguageModel(nn.Module):
    def __init__(self, ):
        ...
        """
        MultiHeadAtttention with head_size = n_embd // n_head. so 4 8-dimensional heads.
    ...
    def forward(self, idx, targets=None):
        ...
        """
        torch.arange just creates a tensor [0, 1, 2, ...., T-1]
        It seems he's just assigning a sequential integer for each position in the sequence.
        The position embedding has shape (T x C) or (number of tokens x embedding dimension)
        ...
        the tutorial to a block of atention.
        """
...
#print the numer of parameters in the model
